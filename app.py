...
with col1:
    ...
    stores = st.number_input("Количество магазинов", min_value=0, max_value=30, value=DEFAULT_STORES, step=1, key="stores_input")
...
with st.expander("Тестирование модели на реальных данных"):
    if st.button("Запустить тест на 5 объектах"):
        try:
            ...
            y_test_original = (y_test.values.flatten() * lambda_opt + 1) ** (1 / lambda_opt)
            ...
            for i in range(min(5, len(X_test))):
                ...
                pred_price = (pred_scaled * lambda_opt + 1) ** (1 / lambda_opt)
